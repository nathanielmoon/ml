...
OUTPUT = OUTPUT_DIR / 'bdt/experiment_2/'
...
def generate_outputs(results, data):
    ...
    plt.title('Hypersphere Boosted DT Accuracy by Min Samples Leaf')
    plt.xlabel('Min Samples Leaf')
    plt.ylabel('Accuracy')
    plt.savefig(OUTPUT / 'btd_accuracy_by_min_samples_leaf.png')
    ...
    reduced_report = reduced_report.pivot(
        index='min_samples_leaf', columns='split', values='precision')
    sns.lineplot(data=reduced_report)
    plt.title('Boosted DT Precision by Min Samples Leaf')
    plt.xlabel('Min Samples Leaf')
    plt.ylabel('Precision')
    plt.savefig(OUTPUT / 'btd_precision_by_min_samples_leaf.png')
    ...
    sns.lineplot(data=reduced_report)
    plt.title('Boosted DT F1 Score by Min Samples Leaf')
    plt.xlabel('Min Samples Leaf')
    plt.ylabel('F1 Score')
    plt.savefig(OUTPUT / 'btd_f1_by_min_samples_leaf.png')
...
def run():
    print("Running BDT Experiment 2 ...")
